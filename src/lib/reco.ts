      ...
      tools: [
        {
          ...
          input_schema: {
            type: "object",
            properties: {
              full_name: { type: "string", description: "Registrant's full name as printed." },
...
/** Lenient name match: extracted text must contain the last name and the first
 *  name (or at least the first initial). Tolerates middle names and order. */
export function recoNameMatches(
  ...
  const ex = normalize(extractedFullName);
  if (!ex) return false;
  const f = normalize(firstName);
  const l = normalize(lastName);
  if (!f || !l) return false;
  const lastOk = ex.includes(l);
  const firstOk = ex.includes(f) || new Set(ex.split(" ")).has(f[0]);
 ...
 * one is already on file), and a name match.
 */
export function recoCertificateApproves(
  ...
  return recoNameMatches(ex.full_name, profile.first_name, profile.last_name);
}
