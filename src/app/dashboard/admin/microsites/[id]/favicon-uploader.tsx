...
import { saveMicrositeFavicon } from "../actions";
...
/** Site-icon control: upload (direct to storage), paste a URL, or clear. */
export function FaviconUploader({
  ...
  async function save(fd: FormData) {
    ...
    const res = await saveMicrositeFavicon(fd);
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (v.error || !v.file) {
      setError(v.error ?? "Pick a PNG, JPG, or WebP.");
      return;
    }
    setBusy(true);
    const path = `${projectId}/favicon-${Date.now()}.${extFor(v.file.type)}`;
    ...
    <div className="space-y-3">
      {currentUrl ? (
        <div className="flex items-center gap-4">
          <span className="inline-flex h-16 w-16 items-center justify-center rounded-lg border border-slate-200 bg-white p-2">
          ...
          <span className="inline-flex h-8 w-8 items-center justify-center rounded border border-slate-200 bg-white p-1">
            {/* eslint-disable-next-line @next/next/no-img-element -- tab-size preview */}
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="min-w-64 flex-1">
          <Input
            ...
            placeholder="…or paste an icon URL (https://…)"
            maxLength={2000}
          />
        </div>
        <Button
