...
export default async function MyProposalsPage({
  ...
  const { data } = await supabase
    ...
    .select(
      "id, proposal_format, commission_ask_percent, price_reduction_ask, incentive_ask, consideration, narrative, valid_until, status, admin_notes, created_at, project_id",
    )
  ...
  // Project names come from the broker-safe view (base `projects` is admin-only).
  ...
  if (projectIds.length > 0) {
    ...
    const byId = new Map(
      (projects ?? []).map((p) => [
        p.id as string,
        { project_name: p.project_name as string, slug: p.slug as string },
      ]),
    );
    for (const r of rows) r.project = byId.get(r.project_id) ?? null;
  }
