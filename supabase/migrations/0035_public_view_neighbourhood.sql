-- 0035_public_view_neighbourhood.sql
--
-- Expose the structured neighbourhood data (migration 0034) on the public
-- consumer surface by adding it to public_projects_view, which the public
-- project page reads. Appended at the end so existing column order/types are
-- unchanged (CREATE OR REPLACE VIEW requirement). The broker dashboard reads
-- public_project_pages directly, so it needs no view change.
